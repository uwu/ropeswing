...
export default defineExtension({
	...
	core: true,
	app: (WApplication: typeof w96.WApplication, components: Record<string, any>) =>
		class RoyalmailApplet extends WApplication {
			constructor() {
				super();
			}
			...
			public main = async (argv: string[]) =>
				useRoot((dispose) => {
					...
					pages.forEach((p) => {
						const page = tabControl.addPage(p.name, async (elem: HTMLElement) =>
							void render(createElement(p.component), elem));
